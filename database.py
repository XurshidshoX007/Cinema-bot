...
# Default to persistent volume path on Railway, local path for development
_db_path_env = os.getenv("DATABASE_PATH", "./movies.db")
PRIMARY_DB_PATH = Path(_db_path_env).resolve()

# Ensure parent directory exists (important for Volume)
PRIMARY_DB_PATH.parent.mkdir(parents=True, exist_ok=True)

# Single database file (no more RUNTIME_DB_PATH, INSPECT_DB_PATH, COPY_DB_PATH confusion)
